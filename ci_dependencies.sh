...
if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/libmagic.git $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/libmagic.git $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/libmagic.git | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/libmagic.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/libmagic.git $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/libmagic.git"
            echo "git clone https://github.com/42ity/libmagic.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/libmagic.git $FOLDER_NAME
        fi
    fi
...
if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/fty-common-db.git $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-common-db.git $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/fty-common-db.git | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/fty-common-db.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/fty-common-db.git $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/fty-common-db.git"
            echo "git clone https://github.com/42ity/fty-common-db.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-common-db.git $FOLDER_NAME
        fi
    fi
...
# Start of recipe for dependency: fty-asset-activator
if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty_asset_activator-dev >/dev/null 2>&1) || \
...
if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/fty-asset-activator $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-asset-activator $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/fty-asset-activator | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/fty-asset-activator $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/fty-asset-activator $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/fty-asset-activator"
            echo "git clone https://github.com/42ity/fty-asset-activator $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-asset-activator $FOLDER_NAME
        fi
    fi
