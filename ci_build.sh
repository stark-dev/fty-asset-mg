...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    ; then
        ...
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 -b 5.18-FTY-master https://github.com/42ity/libmagic.git magic
        cd ./magic
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
    ...
    # Start of recipe for dependency: etn-licensing-credits
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libetn_licensing_credits-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions etn-licensing-credits >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'etn-licensing-credits' from Git repository..." >&2
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 -b master https://bitbucket-prod.tcc.etn.com/scm/bios/etn-licensing-credits.git etn-licensing-credits
        cd ./etn-licensing-credits
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
        if [ -e autogen.sh ]; then
