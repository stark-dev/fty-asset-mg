...
#define GET_NUMBER2(host) { \
    if (self->needle + 2 > self->ceiling) \
        goto malformed; \
    (host) = static_cast<uint16_t>((static_cast<uint16_t>(self->needle [0]) << 8) \
           +  static_cast<uint16_t>(self->needle [1])); \
    self->needle += 2; \
}
    ...
    switch (self->id) {
        ...
        case COMMON_MSG_RETURN_LAST_MEASUREMENTS:
            ...
            if (self->measurements) {
                ...
                char *measurements = static_cast<char *>(zlist_first (self->measurements));
                while (measurements) {
                    frame_size += 4 + strlen (measurements);
                    measurements = static_cast<char *>(zlist_next (self->measurements));
                }
            }
            break;
...
common_msg_print (common_msg_t *self)
{
    ...
    switch (self->id) {
        case COMMON_MSG_GET_MEASURE_TYPE_I:
            ...
            log_debug ("    mt_id=%ld", static_cast<long>(self->mt_id));
            break;
        ...
        case COMMON_MSG_GET_MEASURE_SUBTYPE_I:
            ...
            log_debug ("    mt_id=%ld", static_cast<long>(self->mt_id));
            log_debug ("    mts_id=%ld", static_cast<long>(self->mts_id));
            break;
        ...
        case COMMON_MSG_GET_MEASURE_SUBTYPE_S:
            ...
            if (self->mts_name)
                ...
                log_debug ("    mts_name=");
            log_debug ("    mts_scale=%ld", static_cast<long>(self->mts_scale));
            break;
        ...
        case COMMON_MSG_FAIL:
            ...
            log_debug ("    errtype=%ld", static_cast<long>(self->errtype));
            log_debug ("    errorno=%ld", static_cast<long>(self->errorno));
            if (self->errmsg)
        ...
        case COMMON_MSG_DB_OK:
            ...
            log_debug ("    rowid=%ld", static_cast<long>(self->rowid));
            log_debug ("    aux=");
            if (self->aux) {
                char *item = static_cast<char *>(zhash_first (self->aux));
                while (item) {
        ...
        case COMMON_MSG_UPDATE_CLIENT:
            ...
            log_debug ("    client_id=%ld", static_cast<long>(self->client_id));
            log_debug ("    msg=");
            if (self->msg)
                zmsg_print (self->msg);
            else
        ...
        case COMMON_MSG_NEW_MEASUREMENT:
            ...
            log_debug ("    mt_id=%ld", static_cast<long>(self->mt_id));
            log_debug ("    mts_id=%ld", static_cast<long>(self->mts_id));
            log_debug ("    value=%ld", static_cast<long>(self->value));
            break;

        case COMMON_MSG_CLIENT_INFO:
            log_debug ("COMMON_MSG_CLIENT_INFO:");
            log_debug ("    client_id=%ld", static_cast<long>(self->client_id));
            log_debug ("    device_id=%ld", static_cast<long>(self->device_id));
            log_debug ("    info=[ ... ]");
            log_debug ("    date=%ld", static_cast<long>(self->date));
            break;
        ...
        case COMMON_MSG_DELETE_CINFO:
            ...
            log_debug ("    cinfo_id=%ld", static_cast<long>(self->cinfo_id));
            break;
        ...
        case COMMON_MSG_DEVICE:
            ...
            log_debug ("    devicetype_id=%ld", static_cast<long>(self->devicetype_id));
            if (self->name)
                log_debug ("    name='%s'", self->name);
            else
                log_debug ("    name=");
...
common_msg_aux_string (common_msg_t *self, const char *key, const char *default_value)
{
    ...
    if (self->aux)
        value = static_cast<const char*>(zhash_lookup (self->aux, key));
    if (!value)
        value = default_value;
...
common_msg_measurements_first (common_msg_t *self)
{
    assert (self);
    if (self->measurements)
        return static_cast<const char *>(zlist_first (self->measurements));
    else
        return NULL;
}
...
common_msg_measurements_next (common_msg_t *self)
{
    assert (self);
    if (self->measurements)
        return static_cast<const char *>(zlist_next (self->measurements));
    else
        return NULL;
}
