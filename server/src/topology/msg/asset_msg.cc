...
#define PUT_NUMBER2(host) { \
    self->needle [0] = static_cast<byte>(((host) >> 8)  & 255); \
    self->needle [1] = static_cast<byte>(((host))       & 255); \
    self->needle += 2; \
}

//  Put a 4-byte number to the frame
#define PUT_NUMBER4(host) { \
    self->needle [0] = static_cast<byte>(((host) >> 24) & 255); \
    self->needle [1] = static_cast<byte>(((host) >> 16) & 255); \
    self->needle [2] = static_cast<byte>(((host) >> 8)  & 255); \
    self->needle [3] = static_cast<byte>(((host))       & 255); \
    self->needle += 4; \
}

//  Put a 8-byte number to the frame
#define PUT_NUMBER8(host) { \
    self->needle [0] = static_cast<byte>(((host) >> 56) & 255); \
    self->needle [1] = static_cast<byte>(((host) >> 48) & 255); \
    self->needle [2] = static_cast<byte>(((host) >> 40) & 255); \
    self->needle [3] = static_cast<byte>(((host) >> 32) & 255); \
    self->needle [4] = static_cast<byte>(((host) >> 24) & 255); \
    self->needle [5] = static_cast<byte>(((host) >> 16) & 255); \
    self->needle [6] = static_cast<byte>(((host) >> 8)  & 255); \
    self->needle [7] = static_cast<byte>(((host))       & 255); \
    self->needle += 8; \
}
...
#define GET_NUMBER2(host) { \
    if (self->needle + 2 > self->ceiling) \
        goto malformed; \
    (host) = (static_cast<uint32_t>(self->needle [0]) << 8) \
           +  static_cast<uint32_t>(self->needle [1]); \
    self->needle += 2; \
}
...
#define GET_NUMBER4(host) { \
    if (self->needle + 4 > self->ceiling) \
        goto malformed; \
    (host) = (static_cast<uint32_t>(self->needle [0]) << 24) \
           + (static_cast<uint32_t>(self->needle [1]) << 16) \
           + (static_cast<uint32_t>(self->needle [2]) << 8) \
           +  static_cast<uint32_t>(self->needle [3]); \
    self->needle += 4; \
}
...
#define GET_NUMBER8(host) { \
    if (self->needle + 8 > self->ceiling) \
        goto malformed; \
    (host) = (static_cast<uint64_t>(self->needle [0]) << 56) \
           + (static_cast<uint64_t>(self->needle [1]) << 48) \
           + (static_cast<uint64_t>(self->needle [2]) << 40) \
           + (static_cast<uint64_t>(self->needle [3]) << 32) \
           + (static_cast<uint64_t>(self->needle [4]) << 24) \
           + (static_cast<uint64_t>(self->needle [5]) << 16) \
           + (static_cast<uint64_t>(self->needle [6]) << 8) \
           +  static_cast<uint64_t>(self->needle [7]); \
    self->needle += 8; \
}

//  Put a string to the frame
#define PUT_STRING(host) { \
    uint8_t string_size = static_cast<uint8_t>(strlen (host)); \
...
#define PUT_LONGSTR(host) { \
    uint32_t string_size = static_cast<uint32_t>(strlen (host)); \
    PUT_NUMBER4 (string_size); \
    memcpy (self->needle, (host), static_cast<size_t>(string_size)); \
    self->needle += string_size; \
}
...
{
    asset_msg_t *self = static_cast<asset_msg_t *>(zmalloc (sizeof (asset_msg_t)));
    self->id = id;
    return self;
}
...
asset_msg_encode (asset_msg_t **self_p)
{
    ...
    switch (self->id) {
        ...
        case ASSET_MSG_DEVICE:
            ...
            if (self->groups) {
                ...
                char *groups = static_cast<char *>(zlist_first (self->groups));
                while (groups) {
                    frame_size += 4 + strlen (groups);
                    groups = static_cast<char *>(zlist_next (self->groups));
                }
            }
            ...
            if (self->powers) {
                //  Add up size of list contents
                char *powers = static_cast<char *>(zlist_first (self->powers));
                while (powers) {
                    frame_size += 4 + strlen (powers);
                    powers = static_cast<char *>(zlist_next (self->powers));
                }
            }
        ...
        case ASSET_MSG_RETURN_ELEMENTS:
            ...
            if (self->element_ids) {
                ...
                //  Add up size of dictionary contents
                char *item = static_cast<char *>(zhash_first (self->element_ids));
                while (item) {
                    self->element_ids_bytes += 1 + strlen ((zhash_cursor (self->element_ids)));
                    self->element_ids_bytes += 4 + strlen (item);
                    item = static_cast<char *>(zhash_next (self->element_ids));
                }
            }
    ...
    PUT_NUMBER1 (static_cast<byte>(self->id));
    ...
    if (self->id == ASSET_MSG_DEVICE) {
        if (self->msg) {
            zframe_t *frame1 = zmsg_pop (self->msg);
            while (frame1) {
                zmsg_append (msg, &frame1);
                frame1 = zmsg_pop (self->msg);
            }
        }
        else {
            zframe_t *frame1 = zframe_new (NULL, 0);
            zmsg_append (msg, &frame1);
        }
    }
    //  Now send the message field if there is any
    ...
    switch (self->id) {
        case ASSET_MSG_ELEMENT:
            ...
            if (self->name)
                ...
                log_debug ("    name=");
            log_debug ("    location=%ld", static_cast<long>(self->location));
            log_debug ("    location_type=%ld", static_cast<long>(self->location_type));
            log_debug ("    type=%ld", static_cast<long>(self->type));
            log_debug ("    ext=");
            if (self->ext) {
                char *item = static_cast<char *>(zhash_first (self->ext));
                while (item) {
        ...
        case ASSET_MSG_GET_ELEMENT:
            ...
            log_debug ("    element_id=%ld", static_cast<long>(self->element_id));
            log_debug ("    type=%ld", static_cast<long>(self->type));
            break;
        ...
        case ASSET_MSG_FAIL:
            ...
            log_debug ("    error_id=%ld", static_cast<long>(self->error_id));
            break;
        ...
        case ASSET_MSG_GET_LOCATION_FROM:
            ...
            log_debug ("    element_id=%ld", static_cast<long>(self->element_id));
            log_debug ("    recursive=%ld", static_cast<long>(self->recursive));
            log_debug ("    filter_type=%ld", static_cast<long>(self->filter_type));
            break;
...
asset_msg_ext_string (asset_msg_t *self, const char *key, const char *default_value)
{
    ...
    if (self->ext)
        value = static_cast<const char *>(zhash_lookup (self->ext, key));
    if (!value)
        value = default_value;
...
asset_msg_ext_number (asset_msg_t *self, const char *key, uint64_t default_value)
{
    ...
    if (string)
        value = static_cast<uint64_t>(atol (string));
...
asset_msg_groups_first (asset_msg_t *self)
{
    assert (self);
    if (self->groups)
        return static_cast<char *>(zlist_first (self->groups));
    else
        return NULL;
}
...
asset_msg_groups_next (asset_msg_t *self)
{
    assert (self);
    if (self->groups)
        return static_cast<char *>(zlist_next (self->groups));
    else
        return NULL;
}
...
asset_msg_powers_first (asset_msg_t *self)
{
    assert (self);
    if (self->powers)
        return static_cast<const char *>(zlist_first (self->powers));
    else
        return NULL;
}
...
asset_msg_powers_next (asset_msg_t *self)
{
    assert (self);
    if (self->powers)
        return static_cast<const char *>(zlist_next (self->powers));
    else
        return NULL;
}
...
asset_msg_element_ids_string (asset_msg_t *self, const char *key, const char *default_value)
{
    ...
    const char *value = NULL;
    if (self->element_ids)
        value = static_cast<const char *>(zhash_lookup (self->element_ids, key));
    if (!value)
        value = default_value;
...
asset_msg_element_ids_number (asset_msg_t *self, const char *key, uint64_t default_value)
{
    ...
    char *string = NULL;
    if (self->element_ids)
        string = static_cast<char *>(zhash_lookup (self->element_ids, key));
    if (string)
        value = static_cast<uint64_t>(atol (string));
